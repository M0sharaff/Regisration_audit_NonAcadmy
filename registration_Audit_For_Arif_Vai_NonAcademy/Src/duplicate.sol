...
contract UserRegistration is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        if (_feeCollector == address(0)) revert InvalidAddress();//@audit low  initial owner not checked 
    ...
    function registerUser(
        ...
        user.registrationTime = block.timestamp;
        user.isRegistered = true;
        user.username = _username;//@audit admin can be used still 
    ...
    {
        UserInfo storage user = users[userAddress];//@audit memory pull would be anough 
        return (//@audit revert if non existent 
            user.userId,
            user.referrerId,
